...
def add_records(items):
    for item in items:
        ...
        conn.execute('insert into maria (data, numero, mensagem) values(?, ?, ?)', (dt, item['num'].replace(' ',''), msg) )
        conn.commit()

def get_random():
    c = conn.execute('select id, data, numero, mensagem from maria ORDER BY RANDOM() LIMIT 1;')
    rows = c.fetchall()[0]
    msg = "%s - %s, %s" % (rows[3], rows[2], rows[1])
    return msg

def find_record(find, position = 0):
    sql = 'select count(1) from maria where mensagem like ?'
    args = ['%'+find+'%']
    ...
    sql = 'select id, data, numero, mensagem from maria WHERE mensagem like ? ORDER BY data LIMIT ?,1'
    args = ['%'+find+'%', position]
    c = conn.execute(sql, args)
    rows = c.fetchall()[0]
    msg = "%s - %s, %s" % (rows[3], rows[2], rows[1])
...
def loljews(s):
    ...
    c = conn.execute('select id, data, numero, mensagem from maria where id = ?;', [myid])
    rows = c.fetchall()[0]
    msg = "%s - %s, %s" % (rows[3], rows[2], rows[1])
    return msg
